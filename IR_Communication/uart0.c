...
char getcUart0()
{
    ...
    if(UART0_RSR_R & UART_RSR_OE) //Check RSR register for FIFO overflow flag
    {
        char buffer [8];
        putsUart0("FIFO FULL"); // send out fifo full message
        putsUart0(toAsciiHex(buffer,UART0_RSR_R)); //send out RSR register contents to confirm overflow
    }
